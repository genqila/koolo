...
func MoveTo(dest data.Position, options ...MoveOption) error {
	...
	for {
		...
		} else if auraProvider, ok := ctx.Char.(interface{ MovementAura() skill.ID }); ok {
			if aura := auraProvider.MovementAura(); aura != 0 {
				if kb, found := ctx.Data.KeyBindings.KeyBindingForSkill(aura); found {
					_ = SelectRightSkillByKeyBinding(kb)
				}
			}
		}
