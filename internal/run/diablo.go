...
func (d *Diablo) killSealElite(sealEliteName string) error {
	...
	if sealElite.UnitID == 0 {
		...
		return fmt.Errorf("no seal elite found for %s within %v seconds", sealEliteName, timeout)
	}
	...
	if sealElite.UnitID != 0 {
		for killSealEliteAttempts <= 5 {
			...
			if sealEliteDetected && !sealEliteStillAlive && !sealEliteCorpseFound {
		...
		return fmt.Errorf("no seal elite found for %s within %v seconds", sealEliteName, timeout.Seconds())
	}
