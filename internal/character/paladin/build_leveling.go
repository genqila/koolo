...
type PaladinLeveling struct {
	PaladinBase
	// Cached hash of base pickit rules + dynamic runeword/rune rules to avoid rebuilding every update.
	dynamicPickitRulesSignature uint64
}
...
func (p *PaladinLeveling) updateOptionsForLeveling() {
	...
	enabledRunewordRecipes := p.updateLevelingRunewords()
	p.updateLevelingPickitRules(enabledRunewordRecipes)
