...
func (p *PaladinBase) retrieveDefaultMainSkill() paladinMainSkill {
	smiteLevel := p.SkillLevel(skill.Smite)
	hammerLevel := p.SkillLevel(skill.BlessedHammer)
	fohLevel := p.SkillLevel(skill.FistOfTheHeavens)
...
func (p *PaladinBase) executeFohRotation(monster data.Monster) bool {
	...
	if p.SkillLevel(skill.BlessedHammer) >= paladinMainSkillMinLevel && pack.total-pack.magicImmune >= 3 {
	...
	// Use Blessed Hammer on non-magic-immune if we invested at least 2 hard point points.
	if p.SkillLevel(skill.BlessedHammer) >= paladinMainSkillMinLevel && !monster.IsImmune(stat.MagicImmune) {
		return p.useHammer(monster)
	}
